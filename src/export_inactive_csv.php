<?php
// src/export_inactive_csv.php
require_once '../includes/db_connect.php';
session_start();
$conn = get_db_connection();

if ($conn) {
    // Truy vấn dữ liệu CODE chưa kích hoạt (IsChecked = 0)
    $sql = "SELECT SerialNumber, Code, CustomerName, PhoneNumber, Email, PurchaseLocation, CityProvince, IsChecked, CheckIP, CheckTime FROM CustomerDatabase WHERE IsChecked = 0";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Thiết lập header để tải file CSV với UTF-8 và BOM
        header('Content-Type: text/csv; charset=UTF-8');
        header('Content-Disposition: attachment; filename="customer_database_inactive.csv"');
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $row['CheckTime'] = !empty($row['CheckTime']) && $row['CheckTime'] !== '0000-00-00 00:00:00' 
                ...
                : '';
            // Chuyển IsChecked thành "Chưa Kích Hoạt"
            $row['IsChecked'] = 'Chưa Kích Hoạt';
            fputcsv($output, [
                $row['SerialNumber'],
                $row['Code'],
                $row['CustomerName'],
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Không có dữ liệu CODE chưa kích hoạt để xuất CSV.'
        ]);
    }
